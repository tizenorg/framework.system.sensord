...
int permission_checker::get_permission(int sock_fd)
{
	...
	for (unsigned int i = 0; i < m_permission_infos.size(); ++i) {
		if (!m_permission_infos[i]->need_to_check) {
			...
			ret = security_server_check_privilege_by_sockfd(sock_fd, m_permission_infos[i]->name.c_str(), m_permission_infos[i]->access_right.c_str());
			DBG("security_server_check_privilege_by_sockfd: %d", ret);
