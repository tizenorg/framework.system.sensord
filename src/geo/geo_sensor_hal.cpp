...
#define ELEMENT_MAX_INTERVAL	"MIN_INTERVAL"
...
, m_node_handle(-1)
{
	...
	if (!config.get(SENSOR_TYPE_MAGNETIC, m_model_id, ELEMENT_MIN_RANGE, min_interval)) {
		ERR("[MIN_RANGE] is empty\n");
		throw ENXIO;
	}

	m_min_interval = (float)min_interval;
	INFO("m_min_interval = %f\n",m_min_interval);
...
bool geo_sensor_hal::update_value(bool wait)
{
	...
	if (z)
		m_z =  geo_raw[2];
	if (hdst)
		m_hdst = (geo_raw[3] == 0)? 1 : (geo_raw[3] - 1); /* accuracy bias: -1 */
