...
, m_pir_long(PIR_LONG_STATE_ABSENT)
{
	...
	register_supported_event(PIR_LONG_EVENT_CHANGE_STATE);

	physical_sensor::set_poller(pir_long_sensor::working, this);
}

pir_long_sensor::~pir_long_sensor()
{
	INFO("pir_long_sensor is destroyed!\n");
}
...
{
	pir_long_sensor *sensor = (pir_long_sensor*)inst;
...
bool pir_long_sensor::on_start(void)
{
...
bool pir_long_sensor::on_stop(void)
{
...
int pir_long_sensor::get_sensor_data(unsigned int type, sensor_data_t &data)
{
	int state;

	if (type != PIR_LONG_BASE_DATA_SET)
		return -1;
...
void pir_long_sensor::raw_to_state(sensor_data_t &data)
{
	data.value_count = 1;
}
