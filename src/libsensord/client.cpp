...
{
	retvm_if(!get_sensor_list(), -EIO, "Failed to get sensor list from server");

	vector<sensor_info *> sensor_infos = sensor_info_manager::get_instance().get_infos(type);
	vector<sensor_info *> valid_sensors;
	...
	for (unsigned int i = 0; i < sensor_infos.size(); ++i) {
		if (sensor_infos[0]->get_id() != (sensor_id_t)(-EACCES))
			valid_sensors.push_back(sensor_infos[i]);
	}

	*list = (sensor_t *)malloc(sizeof(sensor_info *) * valid_sensors.size());
	retvm_if(!*list, false, "Failed to allocate memory");

	for (unsigned int i = 0; i < valid_sensors.size(); ++i)
		*(*list + i) = sensor_info_to_sensor(valid_sensors[i]);

	*sensor_count = valid_sensors.size();
