...
sensor_hal_type_t bio_hrm_sensor_hal::get_type(void)
{
	return SENSOR_HAL_TYPE_BIO_HRM;
}
...
bool bio_hrm_sensor_hal::set_interval(unsigned long val)
{
...
bool bio_hrm_sensor_hal::update_value(bool wait)
{
	...
	const int max_heart_rate = 300.0f / m_raw_data_unit;
	int bio_hrm_raw[4] = {0,};
	...
	if (bio_hrm_raw[0] > max_heart_rate) {
		ERR("Drop abnormal HR: %d", bio_hrm_raw[0]);
		return false;
	}
