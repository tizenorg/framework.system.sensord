...
, m_sensorhub_supported(false)
{
	...
	if ((m_node_handle = open(m_resource.c_str(), O_RDWR)) < 0) {
		ERR("Bio_hrm handle open fail for Bio_hrm processor(handle:%d)\n",m_node_handle);
		throw ENXIO;
	}
	...
	INFO("bio_hrm_sensor is created!\n");
...
bio_hrm_sensor::~bio_hrm_sensor()
{
	...
	INFO("bio_hrm_sensor is destroyed!\n");
}

using config::CConfig;

bool bio_hrm_sensor::get_config(const string& element,const string& attr, string& value)
{
	if (m_model_id.empty()) {
	...
	return CConfig::getInstance().get(SENSOR_TYPE_BIO_HRM, m_model_id,element,attr,value);
}

bool bio_hrm_sensor::get_config(const string& element,const string& attr, double& value)
{
	if (m_model_id.empty()) {
		ERR("model_id is empty\n");
		return false;
...
bool bio_hrm_sensor::get_config(const string& element,const string& attr, long& value)
{
	if (m_model_id.empty()) {
		ERR("model_id is empty\n");
		return false;
...
const char *bio_hrm_sensor::name(void)
{
	return m_name.c_str();
}
...
int bio_hrm_sensor::version(void)
{
	return m_version;
}
...
int bio_hrm_sensor::id(void)
{
	return m_id;
}

bool bio_hrm_sensor::update_value(bool wait)
{
	static const int TIMEOUT = 1;
	static const float SNR_SIG_FIGS = 10000.0f;
	int bio_hrm_raw[4] = {0,};
	...
	if (FD_ISSET(m_node_handle, &readfds)) {
		...
		while ((syn == false) && (read_input_cnt < INPUT_MAX_BEFORE_SYN)) {
			...
			if (bio_hrm_input.type == EV_REL) {
				...
				case REL_X:
					bio_hrm_raw[0] = (int)bio_hrm_input.value;
					break;
				case REL_Y:
					bio_hrm_raw[1] = (int)bio_hrm_input.value;
					break;
				case REL_Z:
					bio_hrm_raw[2] = (int)bio_hrm_input.value;
					break;
				default:
				...
				syn = true;
				fired_time = cmodule::get_timestamp(&bio_hrm_input.time);
			} else {
				ERR("bio_hrm_input event[type = %d, code = %d] is unknown.", bio_hrm_input.type, bio_hrm_input.code);
				return false;
			}
	...
	m_hr = bio_hrm_raw[0] - 1;
	m_peek_to_peek = bio_hrm_raw[1] - 1;
...
bool bio_hrm_sensor::is_data_ready(bool wait)
{
	bool ret;
...
bool bio_hrm_sensor::update_name(char *name)
{
	m_name = name;
	return true;
}
...
bool bio_hrm_sensor::update_version(int version)
{
	m_version = version;
	return true;
}
...
bool bio_hrm_sensor::update_id(int id)
{
	m_id = id;
	return true;
}

bool bio_hrm_sensor::update_polling_interval(unsigned long val)
{
	INFO("Not supported setting polling interval, %dms", val);
	return true;
}

bool bio_hrm_sensor::enable_resource(string &resource_node, bool enable)
{
	...
	if (enable) {
		if (m_sensorhub_supported) {
			status = prev_status | (1 << SENSORHUB_BIO_HRM_ENABLE_BIT);
		} else {
			status = 1;
		}
	} else {
...
bool bio_hrm_sensor::start(void)
{
	AUTOLOCK(m_mutex);

	if (m_client > 0) {
		m_client++;
		INFO("Bio_hrm sensor fake starting, client cnt = %d\n", m_client);
		return true;
	}
	...
	m_client = 1;
	m_fired_time = 0;
	INFO("Bio_hrm sensor real starting, client cnt = %d\n", m_client);
	return true;
}



bool bio_hrm_sensor::stop(void)
{

	AUTOLOCK(m_mutex);

	INFO("bio_hrm_sensor client cnt = %d\n", m_client);

	if (m_client > 1) {
		m_client--;
		INFO("Bio_hrm sensor fake stopping, client cnt = %d\n", m_client);
		return true;
	}
	...
	m_client = 0;
	INFO("Bio_hrm sensor real stopping, client cnt = %d\n", m_client);
	return true;
}

int bio_hrm_sensor::get_sensor_type(void)
{
	return m_sensor_type;
}

long bio_hrm_sensor::set_cmd(int type , int property , long input_value)
{
	long value = -1;
...
int bio_hrm_sensor::get_property(unsigned int property_level , void *property_data)
{
	DBG("bio_hrm_sensor called get_property , with property_level : 0x%x", property_level);

	if (property_level == BIO_HRM_BASE_DATA_SET) {
...
int bio_hrm_sensor::get_struct_value(unsigned int struct_type , void *struct_values)
{
	const int chance = 3;
	int retry = 0;
	const int wake_up_time = 10;

	if (struct_type == BIO_HRM_BASE_DATA_SET) {
		while ((m_fired_time == 0) && (retry++ < chance)) {
			INFO("Try usleep for getting a valid BASE DATA value");
			usleep(wake_up_time * 1000llu);
		}
		if (m_fired_time == 0) {
		...
		if ( return_struct_value ) {
			...
			return_struct_value->values_num = 4;
			return_struct_value->values[0] = m_hr;
			return_struct_value->values[1] = m_spo2;
			return_struct_value->values[2] = m_peek_to_peek;
			return_struct_value->values[3] = m_snr;
...
bool bio_hrm_sensor::is_sensorhub_supported(void)
{
	DIR *main_dir = NULL;
...
bool bio_hrm_sensor::check_hw_node(void)
{
	...
	while ((!find_node) && (dir_entry = readdir(main_dir))) {
		if ((strncasecmp(dir_entry->d_name ,".",1 ) != 0) && (strncasecmp(dir_entry->d_name ,"..",2 ) != 0) && (dir_entry->d_ino != 0)) {
			...
			if (CConfig::getInstance().is_supported(SENSOR_TYPE_BIO_HRM,hw_name) == true) {
		...
		while ((!find_node) && (dir_entry = readdir(main_dir))) {
			if (strncasecmp(dir_entry->d_name ,"input",5 ) == 0) {
				...
				if (hw_name == string(INPUT_NAME)) {
					INFO("name_node = %s\n",name_node.c_str());
					DBG("Find H/W  for bio_hrm_sensor\n");
...
{
	bio_hrm_sensor *sample;

	try {
		sample = new bio_hrm_sensor();
	} catch (int ErrNo) {
		ERR("bio_hrm_sensor class create fail , errno : %d , errstr : %s\n",ErrNo, strerror(ErrNo));
		return NULL;
	}
...
{
	bio_hrm_sensor *sample = (bio_hrm_sensor*)inst;
	delete sample;
}
